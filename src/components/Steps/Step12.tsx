...
const Step12 = () => {
  ...
  const y = getY(getValues, {
    content: 'step11.content',
    treatment: 'step11.QVSR',
    control: 'step11.feePreference',
  })

  useMemo(() => {
    if (revisedPrice === LL.choices.revise[1]()) {
      setValue('step12.willingPay', y)
    }
  }, [revisedPrice])
  ...
  return (
    <div css={tw`grid grid-cols-1 gap-6`}>
      ...
      <Typography css={tw`text-justify`}>{LL.questions[423].paragraph2({ y })}</Typography>

      <div>
        <Label required>{LL.questions[423].paragraph3({ y })}</Label>
